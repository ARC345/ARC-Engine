...
namespace ARC {
	COpenGLShader::COpenGLShader(const std::string& _VertexSrc, const std::string& _FragmentSrc)
	{
		...
		if (isCompiled == GL_FALSE)
		{
			...
			ARC_CORE_ASSERT(false, "Vertex compilation failiure!");
			return;
		}
		...
		if (isCompiled == GL_FALSE)
		{
			...
			ARC_CORE_ERROR("{0}", infoLog.data());
			ARC_CORE_ASSERT(false, "Fragment compilation failiure!");
			return;
		}
